...
class MLPModel(models.BaseModel):
  ...
  def create_model(self, model_input, vocab_size, hidden_size=1024, l2_penalty=1e-8, **unused_params):
    ...
    hidden2 = slim.fully_connected(hidden1, hidden_size, activation_fn=tf.nn.relu6, weights_regularizer=slim.l2_regularizer(l2_penalty))

    output = slim.fully_connected(
        hidden2, vocab_size, activation_fn=tf.nn.sigmoid,
        weights_regularizer=slim.l2_regularizer(l2_penalty))
